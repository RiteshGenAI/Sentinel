...
### 🛡️ The Administrator (Admin) Journey
1.  **Centralize Credentials**: The Admin registers Master API keys (e.g., Google Gemini, OpenAI) in the **Providers** interface.
2.  **Define Guardrails**: They create **Projects** representing departments or teams and assign hard **Budgets** (e.g., maximum spent limit of $50).
3.  **Generate Child Keys**: The Admin generates scoped **Child API Keys** bound to specific projects and budgets.
4.  **Oversight**: The Admin views the **Executive Dashboard**, monitors forecasted month-end costs, inspects model usage charts, audits rate limit errors, and exports telemetry logs to CSV.

### 💻 The Project Manager & Developer Journey
1.  **Retrieve Child Keys**: Developers receive a scoped **Child API Key** generated by their Admin or Manager.
...
1.  **Monitor & Analyze**: Viewers (e.g., finance teams or product managers) log in to a read-only view.
2.  **Inspect Telemetry**: They review visual area and bar charts showing token volumes, average cost per model, and project breakdowns.
3.  **Verify Forecasts**: They check the 30-day predictive spending warning levels (`OK`, `WARNING`, `CRITICAL`) to adjust company allocations proactively.
...
1.  Go to the **Providers** workspace.
2.  Click **Add Provider Key** under the Google Gemini section (or your chosen provider).
3.  Provide the master key value and name it.

### 4. Setup a Budget & Project
1.  Navigate to the **Admin** panel (or **Dashboard**).
2.  Create a **Project** (e.g., "Customer Service Chatbot").
3.  Allocate a **Budget** (e.g., Limit: `$10.00`).

### 5. Issue a Scoped Child Key
1.  Return to the **Providers** workspace.
2.  Under the master key you created, click **Issue Scoped Child Key**.
3.  Bind it to the "Customer Service Chatbot" project and copy the newly generated `sk_child_...` key.

### 6. Run Proxy Requests
